...
const RemovePdfPage = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (!f || f.type !== 'application/pdf') {
      setError(t('remove_pdf.please_select_pdf_only'));
      setFile(null);
      return;
    }
    setFile(f);
  ...
  const handleRemovePages = async () => {
   ...
   const token = localStorage.getItem('token');
    ...
    try {
      ...
      if (pages.length === 0) {
        setError(t('remove_pdf.enter_page_numbers'));
        setLoading(false);
        return;
      }
      ...
      const pagesToRemove = pages.map(p => parseInt(p)).filter(p => p > 0 && p <= pageCount);
      if (pagesToRemove.length === 0) {
        setError(t('remove_pdf.invalid_page_numbers', { pageCount }));
        setLoading(false);
        return;
      }
      ...
      // Copy pages except the ones to remove
      ...
      if (err.response?.status === 401) {
        ...
        setError(t('remove_pdf.failed_remove'));
      }
    }
    finally{
      setLoading(false);
  ...
  return (
    <>
        <Helmet>
      <title>Remove PDF Pages Online Free | Delete PDF Pages Fast</title>
      <meta
        name="description"
        content="Easily remove PDF pages online with PDFPivot. Delete unwanted pages from your PDF in seconds. Fast, secure, and 100% free—no installation needed."
      />
    </Helmet>
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-forest mb-2">{t('remove_pdf.title')}</h1>
              ...
              <div className="space-y-6">
                  ...
                  <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gold transition-colors duration-200">
                    <div className="space-y-1 text-center">
                      ...
                      <div className="flex text-sm text-gray-600">
                        <label htmlFor="file-upload-remove" className="relative cursor-pointer bg-white rounded-md font-medium text-gold hover:text-forest focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-gold">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={t('remove_pdf.page_numbers_placeholder')}
                  ...
                  >
                    {loading ? (
                      <>
                        ...
                        {t('remove_pdf.removing')}
                      </>
                    ) : (
                      t('remove_pdf.remove_pages_btn')
                    )}
                  </button>
                </div>
                {/* Download Button */}
