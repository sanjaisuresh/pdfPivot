...
const JpgToPdfPage = () => {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      setError(t("jpg_to_pdf.select_images"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("jpg_to_pdf.success"));
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-forest mb-2">{t("jpg_to_pdf.title")}</h1>
                    ...
                    <div className="space-y-1 text-center">
                      ...
                      <div className="flex text-sm text-gray-600">
                        <span className="relative cursor-pointer bg-white rounded-md font-medium text-gold hover:text-forest focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-gold">
                          {t("jpg_to_pdf.upload_btn")}
                        </span>
                        <p className="pl-1">{t("or_drag_and_drop")}</p>
                      </div>
                      <p className="text-xs text-gray-500">{t("jpg_to_pdf.file_hint")}</p>
                      {files.length > 0 && (
                        <div className="mt-2">
                          <p className="text-sm text-indigo-600 font-medium">
                            {t("jpg_to_pdf.selected_files", { count: files.length })}:
                          </p>
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className={`inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-forest hover:bg-gold hover:text-forest transition duration-200 focus:outline-none focus:ring-2 focus:ring-gold focus:ring-offset-2 ${
                      files.length === 0 || loading ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'
                    }`}
                  >
                    {loading ? (
                      <>
                        ...
                        {t("jpg_to_pdf.converting")}
                      </>
                    ) : (
                      <>
                        <FaFilePdf className="mr-2" />
                        {t("jpg_to_pdf.convert_btn")}
                      </>
                    )}
                  </button>
                </div>
