...
import { FaFilePowerpoint, FaFilePdf, FaDownload } from 'react-icons/fa';
...
const PptToPdfPage = () => {
  ...
  const handleConvert = async () => {
    if (!file) {
      setError(t('ppt_to_pdf.select_ppt'));
      return;
    }
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_BACKEND_BASE_URL+'/api/ppt-to-pdf', {
        method: 'POST',
        body: formData,
      });
      ...
      toast.success(t('ppt_to_pdf.success'));
    } catch (err) {
  ...
  <Helmet>
      ...
      <title>Convert PPT to PDF Online Free | Fast & Secure</title>
      <meta
        name="description"
        content="Convert PPT to PDF with PDFPivot. Turn PowerPoint slides into professional PDFs in seconds. 100% free, secure, and easy—no sign-up or software needed."
      />

      {/* Open Graph (text-only) */}
      <meta property="og:title" content="Convert PPT to PDF Online Free | Fast & Secure" />
      <meta
        property="og:description"
        content="Convert PPT to PDF with PDFPivot. Turn PowerPoint slides into professional PDFs in seconds. 100% free, secure, and easy—no sign-up or software needed."
      />
      <meta property="og:type" content="website" />
      <meta property="og:url" content="https://www.pdfpivot.com/ppt-to-pdf" />

      {/* Twitter Card (text-only summary) */}
      <meta name="twitter:card" content="summary" />
      <meta name="twitter:title" content="Convert PPT to PDF Online Free | Fast & Secure" />
      <meta
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-forest mb-2">{t('ppt_to_pdf.title')}</h1>
                ...
                <div>
                  ...
                  >
                    <div className="space-y-1 text-center">
                      <FaFilePowerpoint className="mx-auto h-12 w-12 text-gray-400" />
                      <div className="flex text-sm text-gray-600">
                        <span className="relative cursor-pointer bg-white rounded-md font-medium text-gold hover:text-forest focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-gold">
                          {t('ppt_to_pdf.upload_btn')}
                        </span>
                        <p className="pl-1">{t("or_drag_and_drop")}</p>
                      </div>
                      <p className="text-xs text-gray-500">{t('ppt_to_pdf.file_hint')}</p>
                      {file && (
                        <div className="mt-2">
                          <p className="text-sm text-indigo-600 font-medium">
                            {t('ppt_to_pdf.selected_file')}:
                          </p>
                  ...
                  >
                    {loading ? (
                      <>
                        ...
                        {t('ppt_to_pdf.converting')}
                      </>
                    ) : (
                      <>
                        <FaFilePdf className="mr-2" />
                        {t('ppt_to_pdf.convert_btn')}
                      </>
                    )}
                  </button>
                </div>
