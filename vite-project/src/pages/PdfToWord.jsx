...
const PdfToWordPage = () => {
  ...
  const handleConvert = async () => {
    if (!file) {
      setError(t('pdf_to_word.select_pdf'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('pdf_to_word.success'));
    } catch (err) {
  ...
  return (<>
   <Helmet>
      {/* SEO Meta Tags */}
      <title>Convert PDF to Word Online Free | Fast & Secure</title>
      <meta
        name="description"
        content="Convert PDF to Word with PDFPivot. Get fully editable DOC files in seconds. Simple, fast, and 100% free—no sign-up or software installation needed."
      />

      {/* Open Graph (text-only) */}
      <meta property="og:title" content="Convert PDF to Word Online Free | Fast & Secure" />
      <meta
        property="og:description"
        content="Convert PDF to Word with PDFPivot. Get fully editable DOC files in seconds. Simple, fast, and 100% free—no sign-up or software installation needed."
      />
      <meta property="og:type" content="website" />
      <meta property="og:url" content="https://www.pdfpivot.com/pdf-to-word" />

      {/* Twitter Card (text-only summary) */}
      <meta name="twitter:card" content="summary" />
      <meta name="twitter:title" content="Convert PDF to Word Online Free | Fast & Secure" />
      <meta
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-forest mb-2">{t('pdf_to_word.title')}</h1>
                  ...
                  >
                    {loading ? (
                      <>
                        ...
                        {t('pdf_to_word.converting')}
                      </>
                    ) : (
                      <>
                        <FaFileWord className="mr-2" />
                        {t('pdf_to_word.convert_btn')}
                      </>
                    )}
                  </button>
                </div>
